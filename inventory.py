...
class Super_inventory():
    ...
    def get_purchase_report(self, purchase_date):
        ...
        report = []
        headers = ['Date', 'Product', 'Qty', 'Unit Cost', 'Total', 'Exp Date', 'Transaction ID']
        report.append(headers)
        total_cost = 0

        for trans_date in self.purchases:
            if purchase_date in trans_date: # check if trans_date contains specific string, bijv:'2021' or '2021-01-14'
    ...
    def get_sales_report(self, sell_date):
        ...
        report = []
        headers = ['Date', 'Product', 'Qty', 'Unit Price', 'Total', 'Transaction ID']
        report.append(headers)
        total_revenue = 0
    ...
    def get_profit_report(self, profit_date):
        ...
        if is_day_report and profit_date in self.sales:
            headers = ['Date', 'Qty x Product', 'Total Cost', 'Total Revenue', 'Total Profit']
            report.append(headers)
            for sale_index in range(len(self.sales[profit_date])):
                ...
                report.append([profit_date, f'{quantity} x {product}', product_cost, product_revenue, product_profit])
        else:
        ...
        for expiry_date in self.expiry_dates:
            ...
            if product_exp_date < current_date: # if expiration date already passed
                for product in self.expiry_dates[expiry_date]:
                    ...
                    report.append([product, quantity, f'EXPIRED on {expiry_date}'])
